...
public class AuthController {
    ...
    public ResponseEntity<?> refreshToken(@RequestBody Object payload) {
        try {
            String refreshToken;
            if (payload instanceof Map<?, ?> map) {
                Object tokenObj = map.get("refreshToken");
                refreshToken = tokenObj != null ? String.valueOf(tokenObj) : null;
            } else {
                refreshToken = payload != null ? String.valueOf(payload) : null;
            }
            if (refreshToken != null) {
                refreshToken = refreshToken.trim();
                // Defensive: strip JSON quotes if body was posted as plain string.
                if (refreshToken.startsWith("\"") && refreshToken.endsWith("\"") && refreshToken.length() >= 2) {
                    refreshToken = refreshToken.substring(1, refreshToken.length() - 1);
                }
            }

            if (refreshToken == null || refreshToken.isBlank()) {
                return ResponseEntity.badRequest().body("Error: Missing refresh token");
            }
