...
public class PaymentController {
    ...
    public ResponseEntity<?> mockConfirmPaymentOrder(
        ...
        return ResponseEntity.ok(paymentService.mockConfirmPayment(bookingId, paymentData, authentication));
    }

    @GetMapping("/dummy-qr/{bookingId}")
    public ResponseEntity<?> dummyQr(@PathVariable Long bookingId, Authentication authentication) {
        try {
            return ResponseEntity.ok(paymentService.buildDummyQrPayload(bookingId, authentication));
        } catch (Exception e) {
            logger.error("Dummy QR failed for booking {}", bookingId, e);
