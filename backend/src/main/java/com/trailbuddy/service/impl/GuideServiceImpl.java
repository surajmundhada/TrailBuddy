...
public class GuideServiceImpl implements GuideService {
    ...
    public Guide registerGuide(GuideProfileDTO guideDTO) {
        ...
        guide.setProfileImageUrl(guideDTO.getProfileImageUrl());
        guide.setIntroVideoUrl(guideDTO.getIntroVideoUrl());
    ...
    public Guide registerGuide(GuideRegistrationDTO registrationDTO, Long userId) {
        ...
        Guide guide = guideRepository.findByUserId(userId).orElseGet(Guide::new);
        ...
        guide.setProfileImageUrl(registrationDTO.getProfileImageUrl());
        guide.setIntroVideoUrl(registrationDTO.getIntroVideoUrl());
    ...
    public Guide updateGuideProfile(Long id, GuideProfileDTO guideDTO) {
        User user = userRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("User not found"));
        Guide guide = resolveGuideProfile(user);
        guide.setBio(guideDTO.getBio());
        guide.setExpertiseAreas(List.of(guideDTO.getExpertise().split(",")));
        guide.setLanguages(List.of(guideDTO.getLanguages().split(",")));
        guide.setHourlyRate(new java.math.BigDecimal(guideDTO.getHourlyRate().toString()));
    ...
    @Override
    public Guide getGuideProfileByUserId(Long userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new RuntimeException("User not found"));
        return resolveGuideProfile(user);
    }
    ...
    @Override
    public Guide resolveGuideProfile(User user) {
        if (user == null) {
            throw new RuntimeException("Guide user is required");
        }

        Optional<Guide> direct = guideRepository.findByUserId(user.getId());
        if (direct.isPresent()) {
            return direct.get();
        }

        List<Guide> nameMatches = guideRepository.findByUserFullName(user.getFirstName(), user.getLastName());
        if (nameMatches.size() == 1) {
            Guide guide = nameMatches.get(0);
            // Self-heal broken demo/dev linkage where the public guide profile exists
            // but is attached to an older duplicate user row.
            guide.setUser(user);
            return guideRepository.save(guide);
        }
